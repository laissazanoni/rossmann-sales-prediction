...
class Rossmann(object):
    ...
    def __init__(self):
        ...
        self.competition_distance_scaler = pickle.load(
            open(os.path.join(self.home_path, 'competition_distance_scaler.pkl'), 'rb'))
        self.competition_time_month_scaler = pickle.load(
            open(os.path.join(self.home_path, 'competition_time_month_scaler.pkl'), 'rb'))
        self.year_scaler = pickle.load(
            open(os.path.join(self.home_path, 'year_scaler.pkl'), 'rb'))
        self.promo_time_week_scaler = pickle.load(
            open(os.path.join(self.home_path, 'promo_time_week_scaler.pkl'), 'rb'))
        self.store_type_scaler = pickle.load(
            open(os.path.join(self.home_path, 'store_type_scaler.pkl'), 'rb'))
    ...
    def data_cleaning(self, df1):
        ...
        # promo2_since_year
        # nulos: loja não possui uma promoção periódica
        df1['promo2_since_year'] = df1.apply(lambda x: x['date'].year if pd.isna(
            x['promo2_since_year']) else x['promo2_since_year'], axis=1)

        # promo2_since_week
        # nulos: loja não possui uma promoção periódica
        df1['promo2_since_week'] = df1.apply(lambda x: x['date'].week if pd.isna(
        ...
        df1['competition_open_since_year'] = df1.apply(lambda x: x['date'].year if pd.isna(
            x['competition_open_since_year']) else x['competition_open_since_year'], axis=1)

        # competition_open_since_month
        # nulos: a loja não possui um competidor mais próxima ou a loja possui um competidor próximo, mas não se sabe quando essa loja abriu
        df1['competition_open_since_month'] = df1.apply(lambda x: x['date'].month if pd.isna(
        ...
        # change data types
        df1.dtypes
        cols = ['competition_open_since_month', 'competition_open_since_year',
    ...
    def feature_engineering(self, df2):
        ...
        # year week
        df2['year_week'] = df2['year'].astype(
            'str') + '-' + df2['week_of_year'].astype('str')
